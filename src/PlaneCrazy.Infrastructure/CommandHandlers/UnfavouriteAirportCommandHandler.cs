...
/// <summary>
/// Command handler for unfavouriting airports.
/// Loads event stream, rebuilds aggregate state, validates command, emits events, and updates projections.
/// </summary>
public class UnfavouriteAirportCommandHandler : ICommandHandler<UnfavouriteAirportCommand>
{
    private readonly IEventStore _eventStore;
    private readonly FavouriteProjection _favouriteProjection;
    private readonly ILogger<UnfavouriteAirportCommandHandler>? _logger;

    public UnfavouriteAirportCommandHandler(
        IEventStore eventStore, 
        FavouriteProjection favouriteProjection,
        ILogger<UnfavouriteAirportCommandHandler>? logger = null)
    {
    ...
    /// <summary>
    /// Handles the UnfavouriteAirport command by:
    ...
    /// </summary>
    public async Task HandleAsync(UnfavouriteAirportCommand command)
    {
        _logger?.LogInformation("Handling UnfavouriteAirport command for {IcaoCode}", 
            command.IcaoCode);
        ...
        try
        {
            ...
            aggregate.UnfavouriteAirport(command);
            ...
            _logger?.LogInformation("Successfully handled UnfavouriteAirport command for {IcaoCode}", 
                command.IcaoCode);
        }
        ...
        {
            _logger?.LogWarning(ex, "Validation failed for UnfavouriteAirport command");
            throw;
        }
        catch (Exception ex)
        {
            _logger?.LogError(ex, "Error handling UnfavouriteAirport command for {IcaoCode}", 
                command.IcaoCode);
            throw;
        }
    }
