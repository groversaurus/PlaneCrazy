...
public class FavouriteAircraftCommandHandler : ICommandHandler<FavouriteAircraftCommand>
{
    ...
    private readonly ILogger<FavouriteAircraftCommandHandler>? _logger;

    public FavouriteAircraftCommandHandler(
        IEventStore eventStore, 
        FavouriteProjection favouriteProjection,
        ILogger<FavouriteAircraftCommandHandler>? logger = null)
    {
    ...
    {
        _logger?.LogInformation("Handling FavouriteAircraft command for {Icao24}", 
            command.Icao24);
        ...
        try
        {
            ...
            _logger?.LogInformation("Successfully handled FavouriteAircraft command for {Icao24}", 
                command.Icao24);
        }
        ...
        {
            _logger?.LogError(ex, "Error handling FavouriteAircraft command for {Icao24}", 
                command.Icao24);
            throw;
        }
    }
